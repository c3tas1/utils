...
def predict_bag(img_paths, backbone, mil_model, device, class_list, transform):
    ...
    for idx, (p_conf, p_pred) in enumerate(zip(p_confs, p_preds)):
        # Conflict: Pill says Class B, Bag says Class A
        if p_pred.item() != bag_pred_idx.item():
            # Only flag if the pill is VERY confident (>95%)
...
def main():
    ...
    if not tasks:
        print("[ERROR] No images found. Please check your input path.")
        sys.exit(1)
