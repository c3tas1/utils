...
# CRITICAL: Disable torch.compile debug logs to clean up your screen
os.environ["TORCH_LOGS"] = "-all"
os.environ["TORCHDYNAMO_VERBOSE"] = "0"

# Enable TF32 for speed on A100 (Keep this!)
...
class GPUAugmentation(nn.Module):
    def __init__(self):
        super().__init__()
        # --- THE "LAZINESS BREAKER" ---
        # We use simple but effective geometric transforms.
        # We removed the heavy ColorJitter that slowed you down,
        # but kept enough to force the model to look at shapes.
        self.transforms = nn.Sequential(
            transforms.RandomHorizontalFlip(),
            transforms.RandomVerticalFlip(), 
            # Randomly rotate +/- 180 degrees (forcing it to learn orientation independence)
...
def save_visual_check(model, dataset, device, epoch, output_dir):
    model.eval()
    # Fast visual check (0 workers to avoid overhead)
...
def validate(model, val_loader, device, gpu_aug):
    ...
    # Parallel Validation Aggregation
...
def main():
    ...
    # Use 8 workers to feed data fast (Safe for 1TB RAM)
    ...
    # Standard CPU Transforms (Resize only)
    ...
    # Critical: Shuffle=False for validation sampler
    ...
    # NO torch.compile here! We rely on standard DDP.
    model = DDP(model, device_ids=[local_rank], find_unused_parameters=False)
    
    # Optimizer with standard Weight Decay
